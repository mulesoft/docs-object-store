...
There’s no limit of the total size of an object store, but each value is limited to 10 MB. The free version also limits usage to 10 transactions per second (TPS). Premium add-on users get up to 100 TPS per app.
