...
Using Object Store v2, a Mule 4 app can access another Mule 4 app using the Object Store Connector for Mule 4.
However, the app can't retrieve the value of the key because it an internal Mule class that is stored as binary.
...
For Mule 3 apps, the key values are stored as type string,
so you can use the https://anypoint.mulesoft.com/exchange/portals/anypoint-platform/f1e97bc6-315a-4490-82a7-23abe036327a.anypoint-platform/object-store-v2/[Object Store v2 REST API] to read from one app and store to another.
...
The maximum TTL (time to live) is 2,592,000 seconds (30 days).

* If you set the TTL in the app, the data persists for the time specified. 

* If you don't set the TTL in the app, the TTL depends on data access:

** If the data is accessed at least once a week in the 30 days after key creation, the TTL is extended for an additional 30 days from the current expiration date.
+
As long as the data continues to be accessed once a week, it is not evicted.
** If the data is not accessed at least once a week, it is evicted in the next 7 to 30 days. 

Specify the TTL in the global configuration parameters for the connector.
