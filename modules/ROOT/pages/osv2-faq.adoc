...
The total size of an object store is not limited, but each value is limited to 10 MB. The free version also limits usage to 10 transactions per second (TPS). Premium add-on users are allowed up to 100 TPS per app.
