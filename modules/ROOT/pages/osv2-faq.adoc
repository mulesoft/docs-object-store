...
* Mule version 4.2.x, 4.3.0, and later:
+
Object stores have a _rolling TTL_ by default.
If the data is accessed at least once a week, the TTL is extended for 30 days.
Otherwise, it is evicted in the next 7 to 30 days.
[NOTE]
Rolling TTL is available for both new and existing apps using Mule version 4.2.x, 4.3.0, and later.
* Mule versions earlier than 4.2.1:
+
Object stores have a TTL of 30 days by default.
