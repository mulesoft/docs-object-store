...
Rate limiting is applied at the Object Store client ID level. When you deploy an app to Runtime Manager and enable Object Store v2, a single client ID is created for the entire application. If your app includes multiple object stores, they all share the same client ID and the same rate limit.

For example, if your app includes both `case-obj-store` and `ticket-obj-store`, Object Store v2 calculates the 100 TPS limit across all stores in the app combined, not per store.
