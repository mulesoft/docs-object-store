...
.6+|Store|`CREATE_STORE` | Create an object store.
| `DELETE_STORE` | Delete an object store.
| `DELETE_ALL_STORES` | Delete all object stores.
| `MODIFY_STORE` | Modify an object store.
| `CLEAR_STORE` | Removes the contents of an object store.
| `RETRIEVE_STORE` | Retrieve the contents of an object store.
.8+|Item| `CLEAR_ITEM` | Clear a key.
| `DELETE_ITEM` | Delete a key or value.
|`CREATE_ITEM` | Create a key or value.
|`MODIFY_ITEM` | Modify a key or value.
|`NACK_ITEM` | Provide negative acknowledgment of a key or value?
|`ACK_ITEM` | Acknowledge...?
|`DELETE_ITEM_AT_INDEX` | Delete the specified key or value?
|`QUERY_ITEM` | Retrieve the contents of a key or value?
|===
