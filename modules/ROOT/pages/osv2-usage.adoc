...
* Each DELETE call counts as a single API request (unit) against the quota.
+
For example, a call to delete a single key-value pair or to delete all key-value pairs in a partion or store counts as one unit against the quota.
