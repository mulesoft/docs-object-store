...
+
Accessing the data at least once a week extends the TTL for 30 days.
...
When you configure a rolling TTL, accessing the data at least once a week extends the TTL for 30 days.
Otherwise, the object store evicts the data in the next 7 to 30 days from the most recent expiration date.
...
If the `entryTtl` value exceeds 30 days, TTL is set to the maximum value of 2592000 seconds (30 days) without returning an error.
