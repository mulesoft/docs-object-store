...
NOTE: Cloud applications using Mule 4 only support the use of Object Store v2. This does not affect on-premises use of Mule 4. Object Store v2 only stores data in the cloud. On-premises use of Mule stores data locally to a persistent data store.
...
* For cloud applications using Mule 4, Object Store v2 overrides the `_defaultPersistentObjectStore` object store. No further configuration is required. If a cloud connector or component uses that object store, then you have access to Object Store v2.
+
For cloud applications using Mule 3, Object Store v2 overrides the `_defaultUserObjectStore` object store. To use Object Store v2 in cloud applications, set the Object Store Reference field to the `_defaultUserObjectStore` value.
