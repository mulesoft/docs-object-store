...
and Mule components, from within a single application.

Object Store v2 acts as the
backend for Mule 3 and Mule 4 CloudHub object store access, which enables applications
...
Object Store v2 can only be used with CloudHub applications. CloudHub applications that use Mule 4 can enable the use of Object Store v2 by setting the Runtime Manager or in-memory Object Store.

On-premise Mule applications do not use Object Store v2 to store data. Instead, you can configure Object Store v2 to store data in memory, in a file system, or in another storage solution in your environment, such as an SQL database.
