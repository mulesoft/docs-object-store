...
You can select Object Store v2 as the implementation for Mule 4 in CloudHub by checking the *Object Store V2* checkbox in Runtime Manager at deployment time.

See the xref:osv2-faq.adoc[FAQ] for Object Store information.

image::osv2-architecture.png["A diagram illustrating how Mule and user-defined components connect to a central data store"]
...
Rolling TTL applies to individual key-value pairs. Each key in a partition tracks its own TTL window. Accessing one key doesn't extend the TTL for other keys in the same partition, and keys expire independently based on their last access time.
...
+
IMPORTANT: Data doesn't carry over from Object Store v1 to v2.  Any data in a v1 object store, including watermarks and other Mule component states, doesn't move to v2. If you revert to Object Store v1, your Object Store v1 data is still there.
+
...
+
This behavior occurs because the "Distributed Locking" feature isn't available in CloudHub and CloudHub 2.0 when working with Object Store v2. As a result, under certain conditions, when multiple Mule components attempt to update Object Store v2 tables simultaneously, such calls may fail.
+
To avoid these issues, use a distributed key-value store as a lock to synchronize access to Object Store v2. For information about distributed locking in Mule runtime engine, see xref:mule-runtime::distributed-locking.adoc[Distributed Locking].

[[os-features-control-plane]]
== Object Store Features by Control Plane
...
[[osv2-on-hyperforce]]
== Object Store v2 on Control Planes
...
When you deploy to Canada Cloud, Japan Cloud, or India Cloud,
all features of Object Store v2 are supported with these exceptions:
