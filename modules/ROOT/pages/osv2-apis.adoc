...
+
--
... Get the client ID (CLIENT_ID) and client secret (CLIENT_SECRET) for the app or connected app.
+
...
Assign the connected app the Organization Administrator scope to access the organization and environment queries.
+
. Run the following command to get the access token from the connected app client ID and client secret:
+
[source,console,linenums]
----
curl --location --request POST https://anypoint.mulesoft.com/accounts/api/v2/oauth2/token \
