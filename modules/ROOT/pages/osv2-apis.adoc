...
* Get the bearer token to access the APIs:
+
--
.. Get the client ID (CLIENT_ID) and client secret (CLIENT_SECRET) for the connected app or client app.
+
...
[source,console,linenums]
----
curl "https://anypoint.mulesoft.com/accounts/api/organizations/{orgId}"
----
...
[source,console,linenums]
----
curl "https://anypoint.mulesoft.com/accounts/api/environments/{environmentId}"
----
...
----
curl -X GET \
  'https://anypoint.mulesoft.com/mq/admin/api/v1/organizations/ORG_ID/environments/ \
   ENV_ID/regions/REGION_ID/destinations/queues/"
----
