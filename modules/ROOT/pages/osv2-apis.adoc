...
+
See <<Get the Access Token for the Anypoint Platform User>>.
...
* <<Get the Access Token for the Anypoint Platform User,Get the access token for the Anypoint Platform user>>.
...
=== Get the Access Token for the Anypoint Platform User 
...
You also need this token to access the Anypoint Platform API to retrieve your environment and organization IDs.

The default time to live (TTL) for the access token is 60 minutes.

To get the access token, use this `curl` command, replacing USERNAME and PASSWORD with the username and password of the Anypoint Platform user:
...
You need the organization ID (ORG_ID) to:

* Get the environment ID.
* Access the Object Store API.
* Retrieve usage metrics by organization with the Object Store Stats API.

To get the organization ID, use this `curl` command, replacing ACCESS_TOKEN with your access token:
...
The organization ID appears in the output as in the following snippet:
...
To get the environment ID, use this `curl` command, replacing ACCESS_TOKEN with your access token 
and ORG_ID with your organization ID:
...
The environment ID appears in the output as in the following snippet:
...
Use the `storeID` in place of STORE_ID in the examples that follow. 

==== Example: Retrieve a List of Partitions

To view the list of partitions, submit a `curl` command like the following, 
replacing BASE_URL with the regional domain, BEARER_TOKEN with your bearer token, 
ORG_ID with your organization ID, 
ENV_ID with your environment ID, and the STORE_ID with the `storeId` from the list of object stores:
...
Use the name of the partition in place of PARTITION_ID in the examples that follow. 

==== Example: Retrieve a List of Keys in a Partition 

To view the keys in a partition, submit a `curl` command like the following, 
replacing BASE_URL with the regional domain, BEARER_TOKEN with your bearer token, 
ORG_ID with your organization ID, 
ENV_ID with your environment ID, the STORE_ID with the name of the object store, and the PARTITION_ID with the name of the partition:
...
Use the value of `keyId` in place of KEY_ID in the examples that follow. 


==== Example: Retrieve the Value of a Key 

To view the value of a key, submit a `curl` command like the following, 
replacing BASE_URL with the regional domain, BEARER_TOKEN with your bearer token, 
ORG_ID with your organization ID, 
ENV_ID with your environment ID, the STORE_ID with the name of the object store, and the PARTITION_ID with the name of the partition, and KEY_ID with the key ID:
...
To store a key-value pair, submit a `curl` command like the following, 
replacing BASE_URL with the regional domain, BEARER_TOKEN with your bearer token, 
ORG_ID with your organization ID, 
ENV_ID with your environment ID, the STORE_ID with the name of the object store, and the PARTITION_ID with the name of the partition, and specify a value for KEY_ID:
...
To delete a key-value pair, submit a `curl` command like the following, 
replacing BASE_URL with the regional domain, BEARER_TOKEN with your bearer token, 
ORG_ID with your organization ID, 
ENV_ID with your environment ID, the STORE_ID with the name of the object store, and the PARTITION_ID with the name of the partition, and the KEY_ID with the key ID:
