...
To get the region ID (REGION_ID), run this command, replacing <<base-url,BASE_URL>>:
...
To get the store ID (STORE_ID), run this command, replacing <<base-url,BASE_URL>>:
...
|Data retention | Length of time that Anypoint MQ retains the data, based on `period`| 
...
* `period=1hour`: 1 month 
* `period=1day`: 6 months
* `period=1month`: 12 months
|===
...
** RAML
** OAS
** Mule 4 or Mule 3 connector file
