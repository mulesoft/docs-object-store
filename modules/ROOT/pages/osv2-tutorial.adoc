...
. Create an app in Design Center, configure Anypoint Object Store Connector, and send data to the object store and view it in Runtime Manager.
. Create an app in Anypoint Studio, configure Anypoint Object Store Connector, and send data to the object store and view it in Runtime Manager.
+
...
** Studio 6 
+
See xref:6.5@studio::to-download-and-install-studio.adoc[Downloading and Installing Anypoint Studio].
--
...
== Create and Configure an App with an Object Store in Design Center
...
+
. Select the *Trigger* box and enter `H` to search for the HTTP Listener:
+
...
+
. In the *Select a component* window, enter `ob` to search for the Object Store Connector:
+
...
+
.The arrow shows the *View in Runtime Manager* option in the *...* menu in Design Center.
...
=== Create a Retrieve Flow to Retrieve the Value
...
.The arrow shows the deployment URL copy icon.
image::os-tutorial-dc-copy-link.png[The copy deployment URL icon]
. Run the following `curl` command from a command prompt to retrieve the key. 
+
Replace _DEPLOYMENT-URL_ with the deployment URL you copied from Design Center. 
+
[source,console]
...
First, you use Studio 7 to configure an app that includes an HTTP Listener trigger and an Object Store Connector.
...
. Start Studio, click *File* > *New* > *Mule Project*, and then name the project `osv2-demo`, and then click *Finish*.
...
. In the *Mule Palette*, click *HTTP*, and then drag the *Listener* to the Studio canvas:
+
...
+
To use Object Store v2 with Mule 4 apps, do not select an object store global element in the *Object store* dropdown.
...
. In the *Connector configuration* dropdown, select the same *HTTP_Listener_config* option that you set up for the Store Listener:
+
.The arrow shows the *Connector configuration* dropdown in the Listener properties window.
image::os-tutorial-studio7-retrieve-listener-props.png[Connector configuration dropdown in the Listener properties window]
...
+
When you are done, the flows should look like this:
+
...
image::os-tutorial-studio6-json-props.png[HTTP properties in Studio 6]
. In the canvas, click *Set Payload*.
. In the Set Payload properties windows, set the *Value* field to `KEY: #[payload.key] AND VALUE: #[payload.value] WERE SAVED`,
+
