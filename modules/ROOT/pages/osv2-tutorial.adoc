...
This document assumes you are familiar with Anypoint Studio, using https://curl.haxx.se/[curl] from a command line, 
...
download and install curl from the net. Alternatively, you can use Postman or browser plugins instead of curl.

You also need to have a Design environment configured in Anypoint Platform > Access Management > Environments. If you don't see Access Management in Anypoint Platform, contact your site 
administrator.
...
of an HTTP Listener trigger and an ObjectStore Connector component. Then you can use
...
+
. In the "Select a component" window, search for "Obj", click the *ObjectStore Connector*, 
...
+
. To the right of the Run button are 3 vertical dots. Click this icon and click *Copy link*.
...
. Open a command line and key in this command:
+
[source,console]
----
curl -X POST -H "Content-Type: application/json" -d '{ "key": "MyKey", "value": "OSv2 Test" }' "http://LINK_FROM_DC/store"
----
+
. Substitute LINK_FROM_DC with the link you copied from Design Center, and then press Enter or Return on your keyboard to send the command to Design Center.
+
...
----
+
. In Anypoint Platform, click the hamburger icon in the upper left and click *Runtime Manager*.
...
You can see that `MyKey` made it into the Object Store user interface, which confirms how to
view that keys successfully can be sent through your Design Center application and stored in 
the Object Store.

Why is the value binary? Mule 4 wraps values into a Mule object that causes 
...
But you probably want clarification that your value worked too. For this we need to return to 
Design Center and add a new flow that retrieves a value.

=== Retrieve the Value

. Click the hamburger icon and click *Design Center*.
...
However you can if you want to verify that the host is 0.0.0.0 and the port is 8081. These 
are the same as in the previous Store flow.
+
. Now that we have started the flow, you can now rename it to Retrieve.
...
In the command prompt, submit this curl command to view the key's value:

[source,console]
----
curl http://osv2-qoav.us-w1.cloudhub.io/retrieve?key=MyKey;echo
----
...
From this exercise, the Object Store user interface helps you
see that the application you created in Design Center can be accessed using a curl command.

For future use of Object Store in Design Center, you don't need to view the Runtime Manager interface each
time you update the Object Store. After you create an application linked to the OSv2 connector, you can
store a key and value in the Object Store with one curl command and view the results
with another. Just change the key and/or the value as needed.

Store:

[source,console]
----
curl -X POST -H "Content-Type: application/json" -d '{ "key": "MyKey", "value": "OSv2 Test" }' "http://osv2-example.us-w1.cloudhub.io/store"
----
...
[source,console]
----
curl http://osv2-qoav.us-w1.cloudhub.io/retrieve?key=MyKey
----
...
+
. In the connector's main screen, set the Path to the `/store` value.
+
...
flow. Use the same settings as in Step 7. In the main screen, set the Path to the
`retrieve` value.
+
image::os-as-2nd-flow-http.png[]
+
. Click ObjectStore in the Mule Palette and drag the *Retrieve* operation to the canvas.
. In the Key field, specify the `#[attributes.queryParams.key]` value.
. Click the green plus sign to the right of the Object Store field, and in the Global
Element Properties screen, click OK to accept the defaults.
. You can change the flow labels by double-clicking the flow labels. Change the first flow to
...
[source,console]
----
$ curl -X POST -H "Content-Type: application/json" -d '{ "key": "MyKey", "value": "OSv2 Test" }' "http://YOUR_CH_URL/store"
----
+
Replace YOUR_CH_URL with the `cloudhub.io` URL for your application in Runtime Manager.
+
...
If you are only working in Anypoint Studio 7, you can skip this section.
