...
. Configure Anypoint Object Store Connector in the app.
...
. From the command line, use `curl` command to send a key-value pair to the object store. 
+
...
+
[IMPORTANT]
When using Object Store in Mule 4, do not select an object store global element in the *Object store* menu.
...
For Studio 7, see <<Create a Mule 4 App with an Object Store in Anypoint Studio 7>>.
