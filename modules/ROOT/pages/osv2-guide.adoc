...
. In Studio, create a simple flow such as:
+
--
image::os-studio7-flow.png["Flow in Studio showing the Listener and Store operations"]
...
. Click *Object Store* in the navigation menu.
. Click the object store, the partition, and then the key:
+
image::os-rtm-key-in-os.png["Runtime Manager window showing the Object Store navigation menu, and the selected object store, partition, key, and key data"]
+
--
[calloutlist]
.. Object Store navigation menu
.. Selected key
.. Key data (binary value and type)
--
+
...
The tutorial provides more detailed information about creating object stores in Mule 4 applications. 
...
. Select the *Store* operation in the flow and, in the properties window, click the green plus icon (+) for the *Object store* field:
+
image::os-use-ttl-props.png["Studio properties window for the Store operation showing the green plus icon"]
. Configure the TTL entries in the *Global Object Store Configuration* window:
+
image::os-use-ttl-global-props.png["Global Object Store Configuration window showing the Entry ttl field"]
+
[NOTE]
For Object Store v2, the *Max entries* field isn't configurable.
+
* *Entry ttl* 
+
For Mule 4.2.1 and later, enter a custom TTL value of 0 through 2592000 seconds (30 days).
+
If *Entry ttl* isn't empty, the TTL is static.
+
...
+
.. In the properties window, click the green plus icon (+) for the *Object store* field:
+
image::os-use-ttl-props.png["Studio properties window for the Store operation showing the green plus icon"]
.. In the *Global Element Properties* window, click the green plus icon (+) for *Configuration Reference* 
to create a global object store configuration:
+
image::os-use-partition-studio7-os-global-props.png["Global Element Properties window showing the green plus icon for the Configuration Reference field"]
...
+
image::os-use-partitions-rtm.png["Runtime Manager window showing Partitions in the Object Store page"]
