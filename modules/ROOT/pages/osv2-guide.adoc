...
If you require a higher level of security, encrypt sensitive data before writing it to Object Store.
...
+
--
.The screenshot shows (*1*) the *Listener* and *Store* operations in the *os-Simple-Flow* flow.
...
+
The `TestKey01` key that you sent from `curl` appears in the object store with the value of `[binary value] BINARY`.
...
+
Similarly, you can delete a partition or the store itself by hovering and clicking the trash can icon.
