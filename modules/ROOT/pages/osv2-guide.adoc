...
== Configure a Custom Time-to-Live
...
* *Entry ttl* 
+
To use rolling TTL, leave this field empty.
Otherwise, enter the custom TTL (between 0 and 2592000 seconds).
If this value is greater than 30 days, TTL is set to the maximum value (2592000 seconds/30 days).
+
For Mule versions earlier than 4.2.1, leave this field empty to set the TTL to the maximum (2592000 seconds).
* *Entry ttl unit*
+
Specify the time interval, such as MINUTES, for the TTL.
* *Expiration interval*
+
...
+
Specify the time unit, such as MINUTES, for the expiration interval.
. Click *OK* to dismiss the *Global Object Store Configuration* window.
